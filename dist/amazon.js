(() => {
  ...
  const productMatchesBook = (book) => {
    ...
    // Amazon bylines and subtitles are inconsistent. A strong title match is enough;
    // otherwise require modest corroboration from the author.
    return titleScore >= 0.88 || (titleScore >= 0.62 && authorScore >= 0.25 && confidence >= 0.58);
  };
  ...
  const waitForPage = (book, checkId) => {
    ...
    const check = () => {
      if (isSearchPage) {
        // Search-result scoring is the match gate. Once a strong result has
        // been selected, Amazon's reformatted product heading must not turn it
        // back into an ambiguous match.
      ...
      if (pageLooksReady && evidence.length) {
